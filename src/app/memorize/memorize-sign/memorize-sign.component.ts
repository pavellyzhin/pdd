...
@Component({
  selector: 'app-memorize-sign',
  templateUrl: './memorize-sign.component.html',
  styleUrls: ['./memorize-sign.component.css']
})
export class MemorizeSignComponent implements OnInit {
  ...
  constructor(
    ...
    this.manageDataService.dataWaitSigns.pipe(map((signs:elementFullModel[])=>{return signs.filter((item)=>item.type.entity.name=='road_sign')})).subscribe((e)=>{
      this.list = e;
